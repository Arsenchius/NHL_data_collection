...
def collect_tables(header: Dict) -> NoReturn:
...
def collect_results(header: Dict, index: int) -> NoReturn:
...
def get_future_games(header: Dict) -> NoReturn:
    ...
    df = pd.DataFrame(results, columns=column_names)
    name = "data/next_tour_games.json"
    df.to_json(name, orient="records")


def collect_data() -> NoReturn:
